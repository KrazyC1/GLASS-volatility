...
class Search(interfaces.plugins.PluginInterface):
    ...
    @classmethod
    def get_requirements(cls) -> interfaces.configuration.RequirementInterface:
        return [
    ...
    def _download_domains(self, domain_type: str) -> list:
        try:
            response = requests.get(DOMAIN_TYPES[domain_type])
    ...
    def _generator(self) -> Iterable[Tuple[int, Tuple[str, str, str, int, str, str]]]:
        ...
        for layer_name in self.context.layers:
            ...
            for offset, match in layer.scan(context=self.context, scanner=scanner):
                domain = next(d for d in domains if d.encode() == match)
                start = max(0, offset - 32)
                end = min(layer.maximum_address, offset + len(match) + 32)
                context = layer.read(start, end - start).decode('utf-8', errors='ignore')
